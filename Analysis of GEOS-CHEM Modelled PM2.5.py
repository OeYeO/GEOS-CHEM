...
STP_T = 293.0  # standard T; 20 deg C
ppm_mgm3 = (
    1 / 22.4 * 273 / STP_T * STP_P / 101325
)  # 1μmol/mol = 1ppm = 1000ppb      1mol/mol = 1*10e3 mmol/mol  1ppm=1.0e-6mol/mol
...
for chosen_scenario in scenarios:
    path = (
        "D:/Jupyter code/GEOS-CHEM/"
        + chosen_scenario
        + "_uk_nest/GEOSChem.SpeciesConc*.nc4"
    )
    # calculate the average PM2.5
    locals()[chosen_scenario] = sorted(glob.glob(path))
    locals()[chosen_scenario + "_species"] = [
        xr.open_dataset(file) for file in vars()[chosen_scenario]
    ]

    for compound in compounds:
        locals()[chosen_scenario + "_" + compound] = [
            data["SpeciesConc_" + compound].isel(time=0, lev=0)
            for data in vars()[chosen_scenario + "_species"]
        ]

        index = compounds.index(compound)
        locals()[chosen_scenario + "_" + compound] = [
            1 * 1.0e9 * var * diag_MWs[index] * ppm_mgm3
            for var in vars()[chosen_scenario + "_" + compound]
        ]
        locals()[chosen_scenario + "_" + compound + "_ave"] = sum(
            locals()[chosen_scenario + "_" + compound]
        ) / len(vars()[chosen_scenario + "_" + compound])

    locals()[chosen_scenario + "_PM25"] = (
        vars()[chosen_scenario + "_NH4_ave"]
        + vars()[chosen_scenario + "_NIT_ave"]
        + vars()[chosen_scenario + "_SO4_ave"]
    ) * 1.35
    +vars()[chosen_scenario + "_BCPI_ave"] + vars()[chosen_scenario + "_BCPO_ave"]
    +(
        vars()[chosen_scenario + "_OCPO_ave"]
        + (vars()[chosen_scenario + "_OCPI_ave"] * 1.07)
    ) * 2.1
    (
        +vars()[chosen_scenario + "_DST1_ave"]
        + vars()[chosen_scenario + "_DST2_ave"] * 0.30
    )
    (
        +vars()[chosen_scenario + "_SALA_ave"] * 1.86
        + vars()[chosen_scenario + "_SOAS_ave"] * 1.07
    )
    ...
    locals()[chosen_scenario + "_PM25"].plot(
        ax=ax, cmap=WhGrYlRd, vmin=4, vmax=13, zorder=0
    )
    ...
    start_lon = [1.8, -8.2, -5.8, -5.8, -1.9, -8.2, -8.2, -5.4, -5.2, 0, 0]
    end_lon = [1.8, 1.8, 1.8, -5.8, -1.9, -8.2, -5.4, -5.4, -5.2, 0, -5.2]
    start_lat = [49.9, 54, 49.9, 54, 54, 54, 55.3, 55.3, 54, 54, 56]
    end_lat = [54, 54, 49.9, 49.9, 49.9, 55.3, 55.3, 54, 56, 56, 56]

    for i in range(len(start_lon)):
        plt.plot(
            [start_lon[i], end_lon[i]],
            [start_lat[i], end_lat[i]],
            color="DarkOrchid",
            linestyle="-",
            linewidth=3,
        )
    ...
    locals()[chosen_scenario + "_PM25_land"] = (
        locals()[chosen_scenario + "_PM25"] * GC_land["uk-mask"]
    )
    locals()[chosen_scenario + "_PM25_ire"] = (
        locals()[chosen_scenario + "_PM25"] * GC_ire["uk-mask"]
    )
    locals()[chosen_scenario + "_PM25_n"] = (
        locals()[chosen_scenario + "_PM25"] * GC_n["uk-mask"]
    )
    locals()[chosen_scenario + "_PM25_sw"] = (
        locals()[chosen_scenario + "_PM25"] * GC_sw["uk-mask"]
    )
    locals()[chosen_scenario + "_PM25_se"] = (
        locals()[chosen_scenario + "_PM25"] * GC_se["uk-mask"]
    )
    locals()[chosen_scenario + "_PM25_land"] = (
        locals()[chosen_scenario + "_PM25_land"].mean().values
    )
    locals()[chosen_scenario + "_PM25_ire"] = (
        locals()[chosen_scenario + "_PM25_ire"].mean().values
    )
    locals()[chosen_scenario + "_PM25_n"] = (
        locals()[chosen_scenario + "_PM25_n"].mean().values
    )
    locals()[chosen_scenario + "_PM25_sw"] = (
        locals()[chosen_scenario + "_PM25_sw"].mean().values
    )
    locals()[chosen_scenario + "_PM25_se"] = (
        locals()[chosen_scenario + "_PM25_se"].mean().values
    )
    locals()[chosen_scenario + "_PM25_land"] = (
        "%.1f" % locals()[chosen_scenario + "_PM25_land"]
    )
    locals()[chosen_scenario + "_PM25_ire"] = (
        "%.1f" % locals()[chosen_scenario + "_PM25_ire"]
    )
    locals()[chosen_scenario + "_PM25_n"] = (
        "%.1f" % locals()[chosen_scenario + "_PM25_n"]
    )
    locals()[chosen_scenario + "_PM25_sw"] = (
        "%.1f" % locals()[chosen_scenario + "_PM25_sw"]
    )
    locals()[chosen_scenario + "_PM25_se"] = (
        "%.1f" % locals()[chosen_scenario + "_PM25_se"]
    )

    # add area annual mean PM2.5 text on figure
    ax.text(
        -0.4,
        58.9,
        f'UK: {locals()[chosen_scenario + "_PM25_land"]}',
        fontsize=50,
        weight="bold",
    ...
    ax.text(
        -9.1,
        53.6,
        f'N.Ireland: {locals()[chosen_scenario + "_PM25_ire"]}',
        fontsize=30,
        weight="bold",
        bbox=dict(facecolor="white", edgecolor="black"),
    )
    ax.text(
        -1.8,
        56.2,
        f'N.England: {locals()[chosen_scenario + "_PM25_n"]}',
        fontsize=30,
        weight="bold",
        bbox=dict(facecolor="white", edgecolor="black"),
    )
    ax.text(
        -1,
        49.5,
        f'SE UK: {locals()[chosen_scenario + "_PM25_se"]}',
        fontsize=30,
        weight="bold",
        bbox=dict(facecolor="white", edgecolor="black"),
    )
    ax.text(
        -5.5,
        49.5,
        f'SW UK: {locals()[chosen_scenario + "_PM25_sw"]}',
        fontsize=30,
        weight="bold",
        bbox=dict(facecolor="white", edgecolor="black"),
    )
    ...
    fig.savefig(
        f"D:/Jupyter code/PM output/{chosen_scenario}_uk_nest.png",
        dpi=100,
        bbox_inches="tight",
    )
...
PM25_change_1 = (
    (locals()["cle_PM25"] - locals()["baseline_PM25"]) / locals()["baseline_PM25"] * 100
)
PM25_change_2 = (
    (locals()["mtf_PM25"] - locals()["cle_PM25"]) / locals()["cle_PM25"] * 100
)
PM25_change_3 = (
    (locals()["mtf_PM25"] - locals()["baseline_PM25"]) / locals()["baseline_PM25"] * 100
)
...
for situation in situations:
    ...
    colorbar.set_ticklabels(
        ["{:.0f}%".format(x) for x in [-60, -40, -20, 0, +20, +40, +60]]
    )
    ...
    if situation == "PM25_change_1":
        ...
        locals()["land_" + situation] = (
            (
                locals()["land_1_" + situation].sum()
                - locals()["land_2_" + situation].sum()
            )
            / locals()["land_2_" + situation].sum()
            * 100
        )
        ...
        locals()["ire_" + situation] = (
            (
                locals()["ire_1_" + situation].sum()
                - locals()["ire_2_" + situation].sum()
            )
            / locals()["ire_2_" + situation].sum()
            * 100
        )
        ...
        locals()["n_" + situation] = (
            (locals()["n_1_" + situation].sum() - locals()["n_2_" + situation].sum())
            / locals()["n_2_" + situation].sum()
            * 100
        )
        locals()["sw_1_" + situation] = locals()["cle_PM25"] * GC_sw["uk-mask"]
        locals()["sw_2_" + situation] = locals()["baseline_PM25"] * GC_sw["uk-mask"]
        locals()["sw_" + situation] = (
            (locals()["sw_1_" + situation].sum() - locals()["sw_2_" + situation].sum())
            / locals()["sw_2_" + situation].sum()
            * 100
        )
        locals()["se_1_" + situation] = locals()["cle_PM25"] * GC_se["uk-mask"]
        locals()["se_2_" + situation] = locals()["baseline_PM25"] * GC_se["uk-mask"]
        locals()["se_" + situation] = (
            (locals()["se_1_" + situation].sum() - locals()["se_2_" + situation].sum())
            / locals()["se_2_" + situation].sum()
            * 100
        )
        # insert title
    ...
    ax.text(
        -9.2,
        53.6,
        f'N.Ireland: {locals()["ire_" + situation]}%',
        fontsize=30,
        weight="bold",
        bbox=dict(facecolor="white", edgecolor="black"),
    )
    ax.text(
        -1.8,
        56.2,
        f'N.England: {locals()["n_" + situation]}%',
        fontsize=30,
        weight="bold",
        bbox=dict(facecolor="white", edgecolor="black"),
    )
    ...
    # save the figure in png.
    fig.savefig(
        f"D:/Jupyter code/PM output/{situation}.png", dpi=100, bbox_inches="tight"
    )
...
for chosen_scenario in scenarios:
    ...
    )  # Display of warnings is completely disabled
    # rename the xarray dataset
    locals()[chosen_scenario + "_PM25"] = locals()[chosen_scenario + "_PM25"].rename(
        chosen_scenario + "_PM25"
    )

    # convert xarray to pandas dataframe
    locals()[chosen_scenario + "_PM25" + "_df"] = xr_to_df(
        locals()[chosen_scenario + "_PM25"]
    )

    # get a copy of full data over PM25_df
    locals()[chosen_scenario + "_PM25" + "_df_test"] = locals()[
        chosen_scenario + "_PM25" + "_df"
    ].copy()
    ...
    for i in range(locals()[chosen_scenario + "_PM25" + "_df_test"].shape[0]):
        locals()[chosen_scenario + "_PM25" + "_df_test"]["poly"][[i]] = create_poly(
            locals()[chosen_scenario + "_PM25" + "_df_test"]["lon"][[i]],
            locals()[chosen_scenario + "_PM25" + "_df_test"]["lat"][[i]],
        )

        locals()[chosen_scenario + "_PM25" + "_df_test"]["poly_group"] = np.nan

    for i in range(locals()[chosen_scenario + "_PM25" + "_df_test"].shape[0]):
        locals()[chosen_scenario + "_PM25" + "_df_test"]["poly_group"][
            [i]
        ] = gpd.GeoSeries(
            locals()[chosen_scenario + "_PM25" + "_df_test"]["poly"][[i]]
        ).intersects(
            Lon_boundary
        )
    ...
    locals()[chosen_scenario + "_PM25" + "_df_test"] = locals()[
        chosen_scenario + "_PM25" + "_df_test"
    ][locals()[chosen_scenario + "_PM25" + "_df_test"]["poly_group"] == True]
    locals()[chosen_scenario + "_PM25" + "_df_test"] = locals()[
        chosen_scenario + "_PM25" + "_df_test"
    ].reset_index(drop=True)

    # calculate mean PM2.5 over these grids
    locals()[chosen_scenario + "_PM25" + "_df_mean"] = locals()[
        chosen_scenario + "_PM25" + "_df_test"
    ][chosen_scenario + "_PM25"].mean()
...
Lon_grids = gpd.GeoSeries(
    unary_union(locals()[chosen_scenario + "_PM25" + "_df_test"]["poly"])
)
...
PM_change_a = (
    (PM25_London_annual_mean[1] - PM25_London_annual_mean[0])
    / PM25_London_annual_mean[0]
    * 100
)
PM_change_b = (
    (PM25_London_annual_mean[2] - PM25_London_annual_mean[1])
    / PM25_London_annual_mean[1]
    * 100
)
...
plt.text(1.4, 4, f"{'%.1f' %PM_change_b}%", c="green", fontsize=19)
...
PM25_D = []
for chosen_scenario in scenarios:
    locals()["PM25_B_" + chosen_scenario] = locals()[chosen_scenario + "_PM25"].sel(
        lat=slice(53.5, 53.9), lon=slice(-1.3125, -0.2)
    )  ### B
    locals()["PM25_C_" + chosen_scenario] = locals()[chosen_scenario + "_PM25"].sel(
        lat=slice(53.2, 53.3), lon=slice(-2.875, -2.5)
    )  ### C
    locals()["PM25_A_" + chosen_scenario] = locals()[chosen_scenario + "_PM25"].sel(
        lat=slice(56, 56), lon=slice(-3.75, -3.75)
    )  ### A
    locals()["PM25_D_" + chosen_scenario] = locals()[chosen_scenario + "_PM25"].sel(
        lat=slice(51.5, 51.5), lon=slice(-3.4375, -3.4375)
    )  ### D
...
plt.savefig(
    "D:/Jupyter code/PM output/annual mean PM2.5 over four polluted areas.jpg", dpi=100
)
...
for chosen_scenario in scenarios:
    for month in range(1, 13):
        ...
        for compound in compounds:
            locals()[chosen_scenario + "_" + compound] = locals()[
                chosen_scenario + "_species"
            ][month - 1]["SpeciesConc_" + compound].isel(time=0, lev=0)
        ...
        locals()[chosen_scenario + "_PM25"] = (
            vars()[chosen_scenario + "_NH4"]
            + vars()[chosen_scenario + "_NIT"]
            + vars()[chosen_scenario + "_SO4"]
        ) * 1.35
        +vars()[chosen_scenario + "_BCPI"] + vars()[chosen_scenario + "_BCPO"]
        +(
            vars()[chosen_scenario + "_OCPO"]
            + (vars()[chosen_scenario + "_OCPI"] * 1.07)
        ) * 2.1
        +vars()[chosen_scenario + "_DST1"] + vars()[chosen_scenario + "_DST2"] * 0.30
        (
            +vars()[chosen_scenario + "_SALA"] * 1.86
            + vars()[chosen_scenario + "_SOAS"] * 1.07
        )

        # calculate monthly PM2.5 over London and four heavily polluted areas
        pandas.set_option("mode.chained_assignment", None)
        # rename the xarray dataset
        locals()[chosen_scenario + "_PM25"] = locals()[
            chosen_scenario + "_PM25"
        ].rename(chosen_scenario + "_PM25")
        # convert xarray to pandas dataframe
        locals()[chosen_scenario + "_PM25" + "_df"] = xr_to_df(
            locals()[chosen_scenario + "_PM25"]
        )
        ...
        locals()[chosen_scenario + "_PM25_monthly_London"].append(
            locals()[chosen_scenario + "_PM25" + "_df_mean"]
        )
        ...
        locals()[chosen_scenario + "_PM25_monthly_B"].append(
            locals()["PM25_B_" + chosen_scenario].mean().values
        )
        locals()[chosen_scenario + "_PM25_monthly_C"].append(
            locals()["PM25_C_" + chosen_scenario].mean().values
        )
        locals()[chosen_scenario + "_PM25_monthly_A"].append(
            locals()["PM25_A_" + chosen_scenario].mean().values
        )
        locals()[chosen_scenario + "_PM25_monthly_D"].append(
            locals()["PM25_D_" + chosen_scenario].mean().values
        )
        ...
        locals()[chosen_scenario + "_PM25"].plot(
            ax=ax, cmap=WhGrYlRd, vmin=4, vmax=22, zorder=0
        )
        ...
        # save figures
        fig.savefig(
            f"D:/Jupyter code/PM output/{chosen_scenario}_seasonal_variation_{month}.png",
            dpi=100,
        )
        plt.close()
...
for area in areas:
    ...
    plt.plot(
        x,
        locals()["baseline" + "_PM25_monthly_" + area],
        linestyle="-",
        linewidth=2,
        color="steelblue",
        marker="s",
    ...
    plt.plot(
        x,
        locals()["cle" + "_PM25_monthly_" + area],
        linestyle="-",
        linewidth=2,
        color="red",
        marker="s",
    ...
    plt.plot(
        x,
        locals()["mtf" + "_PM25_monthly_" + area],
        linestyle="-",
        linewidth=2,
        color="green",
        marker="s",
    ...
    plt.legend(
        ["baseline", "cle", "mtf"], loc="best", fontsize="x-large", frameon=False
    )
...
plt.plot(
    x,
    locals()["baseline" + "_PM25_monthly_London"],
    linestyle="-",
    linewidth=2,
    color="steelblue",
    marker="s",
...
plt.plot(
    x,
    locals()["mtf" + "_PM25_monthly_London"],
    linestyle="-",
    linewidth=2,
    color="black",
    marker="s",
...
plt.savefig(
    f"D:/Jupyter code/PM output/monthly PM2.5 over four polluted areas.jpg", dpi=100
)
